...
class AICheckersState(CheckersState):
        ...
        for r in regular:
            for rd in regular_directions:
                ...
                if self.is_empty_cell(new_loc):
                    all_steps.append({"src": r, "dst": [new_loc]})

        for k in kings:
            for d in CheckersState.all_directions_vectors:
                for j in range(2, 9):
                    new_loc = k[0] + d[0] * j, k[1] + d[1] * j
                    if self.is_step(k, new_loc, KING):
                        all_steps.append({"src": k, "dst": [new_loc]})

        return all_steps

    def get_optimal_killing_for_piece(self, piece, location, captured=None, history=None, all_history=None):
        if captured is None:
            captured = set()
        if history is None:
            history = set()
        if all_history is None:
            all_history = [location]
        valid_captures = self.get_valid_captures(piece, location)
        best_number = len(captured)
        best_moves = [all_history]
        for c in valid_captures:
            ...
            number, moves = self.get_optimal_killing_for_piece(piece, c, new_captured, new_history)
    ...
    def get_best_move(self, depth_remaining):
        ...
        for pm in possible_moves:
            ...
            score, next_move = new_state.get_best_move(depth_remaining - 1)
            if score > best_score:
                best_moves = [pm]
                best_score = score
